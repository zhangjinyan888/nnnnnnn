...
class GUI(tk.Tk):
    ...
    def create_gui(self):
        ...
        self.widget["WebCamStream"] = GE.Button(
            frame,
            "WebCamStream",
            2,
        ...
        parameters_canvas = tk.Frame(
            canvas, style.canvas_frame_label_3, bd=0, width=width, height=1000
        )
    ...
    def load_source_faces(self):
        ...
        try:
            ...
            print(f"Error while loading text embeddings: {e}")
            pass
    ...
    def toggle_play_video(self, set_value="toggle"):
        if self.widget["PreviewModeTextSel"].get() == "Video":
            ...
            if self.widget["TLPlayButton"].get():
                if not self.video_loaded and not self.webcam_on:
    ...
    def set_view(self, _, __):

        self.clear_faces()
        self.video_loaded = False
        self.populate_target_videos()

        print(self.widget["PreviewModeTextSel"].get())
    ...
    def toggle_webcam_stream(self):
        self.widget["WebCamStream"].toggle_button()
        self.control["WebCamStream"] = self.widget["WebCamStream"].get()
        if self.widget["WebCamStream"].get():
            self.webcam_on = True
            self.add_action("play_video", "camera_stream")
        else:
            self.webcam_on = False
