...
class Models:
        ...
        if detect_mode == "Retinaface":
            if not self.retinaface_model:
                self.retinaface_model = onnxruntime.InferenceSession(
                    ".\models\det_10g.onnx", providers=self.providers
                )
            ...
            if not self.scrdf_model:
                self.scrdf_model = onnxruntime.InferenceSession(
                    ".\models\scrfd_2.5g_bnkps.onnx", providers=self.providers
                )
            ...
            if not self.yoloface_model:
                self.yoloface_model = onnxruntime.InferenceSession(
                    ".\models\yoloface_8n.onnx", providers=self.providers
                )
    ...
    def run_align(self, img):
        ...
        if not self.insight106_model:
            self.insight106_model = onnxruntime.InferenceSession(
                ".\models\2d106det.onnx", providers=self.providers
            )
    ...
    def run_recognize(self, img, kps):
        if not self.recognition_model:
            self.recognition_model = onnxruntime.InferenceSession(
                ".\models\w600k_r50.onnx", providers=self.providers
            )
    ...
    def run_swapper(self, image, embedding, output):
        if not self.swapper_model:
            ...
            self.swapper_model = onnxruntime.InferenceSession(
                "./models/inswapper_128.fp16.onnx", providers=self.providers
            )
    ...
    def run_GFPGAN(self, image, output):
        if not self.GFPGAN_model:
            ...
            self.GFPGAN_model = onnxruntime.InferenceSession(
                "./models/GFPGANv1.4.onnx", providers=self.providers
            )
    ...
    def run_GPEN_512(self, image, output):
        if not self.GPEN_512_model:
            self.GPEN_512_model = onnxruntime.InferenceSession(
                "./models/GPEN-BFR-512.onnx", providers=self.providers
            )
    ...
    def run_GPEN_256(self, image, output):
        if not self.GPEN_256_model:
            self.GPEN_256_model = onnxruntime.InferenceSession(
                "./models/GPEN-BFR-256.onnx", providers=self.providers
            )
    ...
    def run_codeformer(self, image, output):
        if not self.codeformer_model:
            self.codeformer_model = onnxruntime.InferenceSession(
                "./models/codeformer_fp16.onnx", providers=self.providers
            )
    ...
    def run_occluder(self, image, output):
        if not self.occluder_model:
            self.occluder_model = onnxruntime.InferenceSession(
                "./models/occluder.onnx", providers=self.providers
            )
    ...
    def run_faceparser(self, image, output):
        if not self.faceparser_model:
            self.faceparser_model = onnxruntime.InferenceSession(
                "./models/faceparser_fp16.onnx", providers=self.providers
            )
    ...
    def resnet50(self, image, score=0.5):
        if not self.resnet50_model:
            self.resnet50_model = onnxruntime.InferenceSession(
                "./models/res50.onnx", providers=self.providers
            )
        ...
        conf = torch.empty(
            (1, 10752, 2), dtype=torch.float32, device="cuda"
        ).contiguous()
        landmarks = torch.empty(
            (1, 10752, 10), dtype=torch.float32, device="cuda"
        ).contiguous()
