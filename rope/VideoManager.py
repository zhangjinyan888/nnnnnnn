...
class VideoManager:
    def __init__(self, models):
        ...
        self.webcam_on = False # flag for webcam state
        ...
        if self.is_video_loaded:
            ...
            if success:
                ...
                if not self.control["SwapFacesButton"]:
                    ...
                    temp = [
                        self.swap_video(target_image, self.current_frame, marker),
                        self.current_frame,
                    ]  # temp = RGB
            ...
            if not self.control["SwapFacesButton"]:
                ...
                temp = [
                    self.swap_video(self.image, self.current_frame, False),
                    self.current_frame,
                ]  # image = RGB
    ...
    def play_video(self, command):
        if command == "play":
            ...
            if self.control["AudioButton"]:
                ...
                self.audio_sp = subprocess.Popen(
            ...
            if not self.webcam_on:
                ...
                self.file_name = "stream_" + str(time.time())[:10]
                self.temp_file = self.output + "temp_" + self.file_name
            ...
            if self.parameters["RecordTypeTextSel"] == "FFMPEG":
                args = [
                    ...
                    "libx264",
                    "-crf",
                    str(self.parameters["VideoQualSlider"]),
                    "-r",
                    str(self.fps),
                    "-s",
            ...
            # self.capture = cap = cv2.VideoCapture('udp://127.0.0.1:10800',cv2.CAP_FFMPEG)

    # @profile
    def process(self):
        process_qs_len = range(len(self.process_qs))
        # print(self.play, self.is_video_loaded, self.is_image_loaded)

        # Add threads to Queue
        if self.play and self.is_video_loaded:
        ...
        elif self.play and not self.is_video_loaded and not self.is_image_loaded:
            # Camera stream mode
            ...
            if success:
                ...
                if self.control["SwapFacesButton"]:
                    frame = self.swap_video(frame, 0, False)
                temp = [frame, 0]
                self.frame_q.append(temp)
                ...
                if self.process_qs[index]["Status"] == "finished":
                    ...
                    if (
                        ...
                        if not os.path.exists(self.temp_file):
                            print(
                                f"Error: Temporary file {self.temp_file} does not exist."
                            )
                            return
                        if not os.path.exists(orig_file):
                            print(f"Error: Input file {orig_file} does not exist.")
                            return
                        if not self.webcam_on: # audio not implemented for webcam yet
                            print("adding audio...")
                            args = [
                                "ffmpeg",
                                "-hide_banner",
    ...
    def swap_video(self, target_image, frame_number, use_markers):
        ...
        for i in range(kpss.shape[0]):
            ...
            face_emb, _ = self.func_w_test(
                "recognize", self.models.run_recognize, img, face_kps
            )
        ...
        if ret:
            ...
            if not control["MaskViewButton"] and parameters["OrientSwitch"]:
                ...
                img = transforms.functional.rotate(
                    img, angle=-parameters["OrientSlider"], expand=True
                )
                img = img.permute(1, 2, 0)
    ...
    def swap_core(self, img, kps, s_e, parameters, control):  # img = RGB
        ...
        if parameters["FaceAdjSwitch"]:
            original_face_128 = v2.functional.affine(
                ...
                0,
                (0, 0),
                1 + parameters["FaceScaleSlider"] / 100,
                0,
                center=(63, 63),
                interpolation=v2.InterpolationMode.BILINEAR,
            )
        ...
        latent = (
            torch.from_numpy(self.models.calc_swapper_latent(s_e)).float().to("cuda")
        )
        ...
        if parameters["StrengthSwitch"]:
            itex = ceil(parameters["StrengthSlider"] / 100.0)
        ...
        gauss = transforms.GaussianBlur(
            parameters["BorderBlurSlider"] * 2 + 1,
        ...
        if parameters["DiffSwitch"]:
            mask = self.apply_fake_diff(
                swap, original_face_512, parameters["DiffSlider"]
            )
            # mask = t128(mask)
            gauss = transforms.GaussianBlur(
                parameters["BlendSlider"] * 2 + 1, (parameters["BlendSlider"] + 1) * 0.2
            )
        ...
        if parameters["FaceParserSwitch"]:
            mask = self.apply_face_parser(
                swap, parameters["FaceParserSlider"], parameters["MouthParserSlider"]
            )
            mask = t128(mask)
            swap_mask = torch.mul(swap_mask, mask)
        ...
        if parameters["CLIPSwitch"]:
            with lock:
                mask = self.func_w_test(
                    ...
                    self.apply_CLIPs,
                    original_face_512,
                    parameters["CLIPTextEntry"],
                    parameters["CLIPSlider"],
                )
    ...
    def apply_occlusion(self, img, amount):
        ...
        outpred = torch.ones(
            (256, 256), dtype=torch.float32, device=device
        ).contiguous()
    ...
    def apply_face_parser(self, img, FaceAmount, MouthAmount):
        ...
        if MouthAmount < 0:
            ...
            for i in range(iters):
                mouth_parse = torch.nn.functional.conv2d(
                    mouth_parse, kernel, padding=(1, 1)
                )
        ...
        if FaceParserAmount != 0:
            ...
            if FaceParserAmount > 0:
                ...
                for i in range(int(FaceParserAmount)):
                    outpred = torch.nn.functional.conv2d(
                        outpred, kernel, padding=(1, 1)
                    )
                    outpred = torch.clamp(outpred, 0, 1)
    ...
    def apply_restorer(self, swapped_face_upscaled, parameters):
        ...
        outpred = torch.add(
            torch.mul(outpred, alpha), torch.mul(swapped_face_upscaled, 1 - alpha)
        )
